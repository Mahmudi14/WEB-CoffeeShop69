...
@section('header-title', 'Kelola Kasir')
...
@section('content')
    <div class="space-y-6">
            ...
            <div class="relative z-10 flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
                <div>
                    ...
                    <p class="mt-2 text-sm leading-6 text-stone-300 max-w-3xl">
                        Superadmin mengatur akun kasir, status aktif, dan memantau apakah kasir sedang memiliki shift aktif.
                    </p>
                </div>

                <a href="{{ route('superadmin.cashiers.create') }}"
        ...
        <section class="rounded-[2rem] border border-stone-100 bg-white p-6 shadow-sm">
            <form action="{{ route('superadmin.cashiers.index') }}" method="GET">
                            ...
                            @foreach ($cashiers as $cashier)
                                <tr class="hover:bg-stone-50">
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        <p class="mt-1 text-xs font-semibold text-stone-500">
                                            {{ $cashier->phone ?: 'Nomor HP belum diisi' }}
                                        </p>
                                    </td>

                                    <td class="px-6 py-4">
                                        @if ($cashier->activeCashierShift)
                                            <span
                                        ...
                                        @else
                                            <span
                                                class="inline-flex rounded-full bg-stone-100 px-3 py-1 text-xs font-black text-stone-600">
                                                Tidak Ada Shift
                                            </span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex justify-end gap-2">
                                            <a href="{{ route('superadmin.cashiers.edit', $cashier) }}"
                                            ...
                                            <form method="POST"
                                                action="{{ route('superadmin.cashiers.toggle-status', $cashier) }}">
                                                @csrf
                                                @method('PATCH')
